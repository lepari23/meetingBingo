...
document
    ...
    .addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); addWord(); } });

/* --------------  SAVE / CLEAR  -------------------------------------- */
function saveNewList() {
    const name = document.getElementById("newListName").value.trim();
    if (!name) { alert("Give your list a name."); return; }
    if (customWords.length < 9) { alert("Need at least 9 words for a 3×3 board."); return; }
...
function updateWordListDisplay() {
    ...
    customWords.forEach((w, i) => {
        ...
        li.textContent = w;
        li.onclick = () => {                                       // inline edit on click
            const inp = document.createElement("input");
            inp.value = w;
            inp.onblur = commit;
            inp.onkeydown = e => { if (e.key === "Enter") commit(); };
            li.innerHTML = ""; li.appendChild(inp); inp.focus();
...
function refreshDownloadSelect() {
    ...
    sel.innerHTML = '<option value="" disabled selected>Select list to download</option>';
...
document.getElementById("uploadFile").addEventListener("change", e => {
    ...
    reader.onload = () => {
        try {
            ...
            let n = 1; while (localStorage.getItem(key)) key = `${key}_${n++}`;
            localStorage.setItem(key, JSON.stringify(arr));
            alert(`Imported “${key}” (${arr.length} words).`);
            listSavedLists(); refreshDownloadSelect();
        } catch { alert("Invalid JSON; must be an array of strings."); }
    };
